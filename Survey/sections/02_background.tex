...
In a standard Retrieval-Augmented Generation (RAG) pipeline, automated evaluation typically relies on the LLM-as-a-Judge paradigm. The baseline workflow is highly structured: the evaluating model receives a user query, the retrieved reference documents, and the target model's generated response. Its primary task is to score the response across specific dimensions, particularly verifying if the generated text is strictly faithful to the provided context. While baseline LLM judges exhibit general vulnerabilities such as verbosity or self-enhancement biases \citep{wang2023fair}, applying this paradigm to dynamic RAG environments introduces a distinct set of domain-specific stressors. 

Overall, this survey adopts a unified view to explain how evaluation reliability is challenged under these RAG-specific pressures. We specifically examine the ``internal fights'' within the model when its own parametric memory conflicts with the retrieved data. Finally, we demonstrate why the field must move toward ``Agent'' systems to resolve these systemic problems. 
...
\begin{figure*}[t]
    ...
    \begin{forest}
      for tree={
        grow'=0, draw, thick, rounded corners=3pt, align=center, font=\sffamily,
        ...
        l sep=8mm, s sep=2.5mm,
      },
      % 定義葉節點(論文)的通用樣式：與父節點一致的粗框與圓角，但設定寬度與左對齊
      leaf style/.style={
        text width=7.5cm, align=left, font=\sffamily\scriptsize
      }
      [Reliability of\\LLM-as-a-Judge\\in RAG Systems, fill=gray!15, font=\sffamily\bfseries
        % --- 第一分支：環境誘因 (藍色系) ---
